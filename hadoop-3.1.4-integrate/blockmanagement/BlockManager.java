 ...
 * distributed under the License is dwistributed on an "AS IS" BASIS,
...
/**
 * Keeps information related to the blocks stored in the Hadoop cluster. For
 * block state management, it tries to maintain the safety property of "# of
 * live replicas == # of expected redundancy" under any events such as
 * decommission, namenode failover, datanode failure.
 *
 * The motivation of maintenance mode is to allow admins quickly repair nodes
 * without paying the cost of decommission. Thus with maintenance mode, # of
 * live replicas doesn't have to be equal to # of expected redundancy. If any of
 * the replica is in maintenance mode, the safety property is extended as
 * follows. These property still apply for the case of zero maintenance
 * replicas, thus we can use these safe property for all scenarios. a. # of live
 * replicas >= # of min replication for maintenance. b. # of live replicas <= #
 * of expected redundancy. c. # of live replicas and maintenance replicas >= #
 ...
 * DFS_NAMENODE_REPLICATION_MIN_KEY. For erasure encoding, # of min live
 * replicas for maintenance is BlockInfoStriped#getRealDataBlockNum.
 *
...
public class BlockManager implements BlockStatsMXBean {
    //-[linesyao]
    private Boolean isRepairBoost = null;
    ///
    ...
    /**
     * redundancyRecheckInterval is how often namenode checks for new reconstruction
     * work.
     */
    private final long redundancyRecheckIntervalMs;

    /**
     * Tracks how many calls have been made to chooseLowReduncancyBlocks since the
     * queue position was last reset to the queue head. If CallsSinceReset crosses
     * the threshold the next call will reset the iterators. A threshold of zero
     * means the queue position will only be reset once the next of the queue has
     * been reached.
     */
    ...
    /**
     * Mapping: Block -> { BlockCollection, datanodes, self ref } Updated only in
    ...
    /**
     * Blocks to be invalidated. For a striped block to invalidate, we should track
     * its individual internal blocks.
     */
    private final InvalidateBlocks invalidateBlocks;

    /**
     * After a failover, over-replicated blocks may not be handled until all of the
     * replicas have done a block report to the new active. This is to make sure
     * that this NameNode has been notified of all block deletions that might have
     * been pending when the failover happened.
     */
    ...
    /**
     * Store set of Blocks that need to be replicated 1 or more times. We also store
     * pending reconstruction-orders.
     */
    ...
    /**
     * When running inside a Standby node, the node may receive block reports from
     * datanodes before receiving the corresponding namespace edits from the active
     * NameNode. Thus, it will postpone them for later processing, instead of
     * marking the blocks as corrupt.
     */
    private boolean shouldPostponeBlocksFromFuture = false;

    /**
     * Process reconstruction queues asynchronously to allow namenode safemode exit
    ...
    /**
     * Minimum live replicas needed for the datanode to be transitioned from
    ...
    public BlockManager(final Namesystem namesystem, boolean haEnabled, final Configuration conf) throws IOException {
        //-[linesyao]
        this.isRepairBoost = conf.getBoolean(DFSConfigKeys.DFS_DN_EC_REPAIRBOOST_KEY, DFSConfigKeys.DFS_DN_EC_REPAIRBOOST_DEFAULT);
        ///
        ...
        this.redundancyRecheckIntervalMs = conf.getTimeDuration(
                ...
                DFSConfigKeys.DFS_NAMENODE_REDUNDANCY_INTERVAL_SECONDS_DEFAULT, TimeUnit.SECONDS,
        ...
        if (minMaintenanceR < 0) {
            throw new IOException("Unexpected configuration parameters: "
                    + DFSConfigKeys.DFS_NAMENODE_MAINTENANCE_REPLICATION_MIN_KEY + " = " + minMaintenanceR + " < 0");
        }
    ...
    private static BlockTokenSecretManager createBlockTokenSecretManager(final Configuration conf) throws IOException {
        ...
        if (isHaEnabled) {
            ...
            return new BlockTokenSecretManager(updateMin * 60 * 1000L, lifetimeMin * 60 * 1000L, nnIndex, nnIds.size(),
                    null, encryptionAlgorithm, shouldWriteProtobufToken, shouldWrapQOP);
        } else {
    ...
    private void dumpBlockMeta(Block block, PrintWriter out) {
        ...
        chooseSourceDatanodes(blockInfo, containingNodes, containingLiveReplicasNodes, numReplicas,
    ...
    /**
     * Commit the last block of the file and mark it as complete if it has meets the
     ...
     * pendingReconstruction in order to keep RedundancyMonitor from scheduling the
     * block.
     */
     ...
     * @param iip      - INodes in path to file containing curBlock; if null, this
    ...
    /**
     * Convert a specified block of the file to a complete block. Skips validity
     * checking and safe mode block total updates; use
    ...
    /**
     * Force the given block in the given file to be marked as complete, regardless
     * of whether enough replicas are present. This is necessary when tailing edit
     * logs as a Standby.
     */
     ...
     * partial block (its size is less than the preferred block size). The converted
     * block is returned to the client. The client uses the returned block locations
     * to form the data pipeline for this block.<br>
     * The methods returns null if there is no partial block at the end. The client
     * is supposed to allocate a new block with the next call.
     *
     * @param bc            file
     ...
     * system configuration and throw an exception if it's not.
     *
     ...
     * @throws java.io.IOException thrown if the requested replication factor is out
     *                             of bounds
     */
     ...
     * @param reason    a textual reason why the block should be marked corrupt, for
     *                  logging purposes
     */
     ...
     * 
     * @param b           Indicating the reported bad block and the corresponding
     *                    BlockInfo stored in blocksMap.
     ...
     * 
     * @return true if the block was successfully invalidated and no longer present
     *         in the BlocksMap
     */
    ...
    int computeBlockReconstructionWork(int blocksToProcess) {
        ...
        try {
            ...
            blocksToReconstruct = neededReconstruction.chooseLowRedundancyBlocks(blocksToProcess, reset); // [linesyao]
        } finally {
    ...
    /**
     * Reconstruct a set of blocks to full strength through replication or erasure
     * coding
     *
    ...
    int computeReconstructionWorkForBlocks(List<List<BlockInfo>> blocksToReconstruct) {
            ...
            synchronized (neededReconstruction) {
                for (int priority = 0; priority < blocksToReconstruct.size(); priority++) {
                    for (BlockInfo block : blocksToReconstruct.get(priority)) {
                        List<DatanodeStorageInfo> bStorageInfos = Lists.newArrayList(block.getStorageInfos());
                        // LOG.info("LinesYao for blk " + block.getBlockName());
                        for(int i=0; i<bStorageInfos.size(); ++i) {
                            int indexInStripe = block.findStorageInfo(bStorageInfos.get(i));
                            // LOG.info("LinesYao index: " + indexInStripe + "ip: " +bStorageInfos.get(i).getDatanodeDescriptor().getIpAddr());
                        }
                    }
                }
            }
        ...
        // -[linesyao]
        if(reconWork.size() > 0 && isRepairBoost) {
            List<BlockReconstructionWork> orderedReconWork = new LinkedList<>();
            List<DatanodeDescriptor> deadDNs = getDatanodeManager().getDatanodeListForReport(DatanodeReportType.DEAD);
            if(deadDNs.size() > 0) {
                // deadDNs > 0 
                blockLog.info("LinesYao dead dns size = " + deadDNs.size());
                for(int i=0; i<deadDNs.size();++i) {
                    blockLog.info("LinesYao dead datanode: " + deadDNs.get(i).getIpAddr());
                }
                blockLog.info("LinesYao in computeReconstructionWorkForBlocks" + " reconWork.size = " + reconWork.size());
                for (int i = 0; i < blocksToReconstruct.size(); ++i) {
                    blockLog.info("LinesYao blocksToReconstruct piority " + i + " , size = " + blocksToReconstruct.get(i).size());
                }
                //- find node failure, act as LinesFullNodeClient and send command to standalone coordinator (sendFullNodeRequest)
                JedisPool _coordinatorJedisPool = null;
                int _coordinatorJedisPoolSize = 2;
                String _coordinatorIp = DFSConfigKeys.DFS_DN_EN_REPARIBOOST_COORDINATOR;
                LOG.info("Linesyao coor " + _coordinatorIp);
                JedisPool _localJedisPool = null;
                int _localJedisPoolSize = 2;
                ...
                */
                for(int i=0; i<deadDNs.size(); ++i) {
                    ...
                    String failedDNIp = deadDNs.get(i).getIpAddr();
                    byte[] failedDNIpAddr = new byte[8];
                    try {
                        InetAddress addr = InetAddress.getByName(failedDNIp);
                        failedDNIpAddr = addr.getAddress();
                    } catch(Exception e) {
                        LOG.info("Exception in inet addr");
                    }
                    LOG.info("LinesYao failedDNIp: " + failedDNIp + " len = " + failedDNIpAddr.length);

                    byte[] cmd = new byte[4+failedDNIpAddr.length];
                    int type = 1;
                    cmd[0] = (byte) (type); 
                    cmd[1] = (byte) (type >> 8); 
                    cmd[2] = (byte) (type >> 16); 
                    cmd[3] = (byte) (type >> 24); 
                    System.arraycopy(failedDNIpAddr, 0, cmd, 4, failedDNIpAddr.length);


                    try (BinaryJedis jedis = _localJedisPool.getResource()) {
                    // try (BinaryJedis jedis = _coordinatorJedisPool.getResource()) {
                        jedis.rpush("dr_request".getBytes(), cmd);
                        LOG.info("LinesYao send dr_request with fullNodeRequest for ip " + failedDNIp);

                    } catch (Exception e) {
                        LOG.info("Exception sendFullNodeRequest");
                    }

                }

                // -- waiting for targets info from LinesCoordinator
                
                Map<String, String> blkName2TargetIp = new HashMap<String, String>();
                Map<String, BlockReconstructionWork> blkName2ReconWork = new HashMap<String, BlockReconstructionWork>();
                for (BlockReconstructionWork rw : reconWork) {
                    String blk = rw.getBlock().getBlockName();
                    blkName2ReconWork.put(blk, rw);
                }
                // try (BinaryJedis jedis = _coordinatorJedisPool.getResource()) {
                try (BinaryJedis jedis = _localJedisPool.getResource()) {
                    String targetIP = "";
                    for(int i=0; i<reconWork.size(); ++i) {
                        List<byte[]> l_blkName = jedis.blpop(0, "HDFS3_blkName".getBytes());
                        String blkName = new String(l_blkName.get(1));
                        LOG.info("LinesYao blkName " + i + " " + blkName);
                        
                        targetIP = "";
                        List<byte[]> l_target = jedis.blpop(0, "HDFS3_target".getBytes());
                        byte[] rsp = l_target.get(1);
                        for(int j=0; j<rsp.length; ++j) {
                            targetIP += (rsp[j] & 0xFF);
                            if(j<3) targetIP += ".";
                        }
                        LOG.info("LinesYao targetIP  " + i + " " + targetIP + " for blk " + blkName);
                        blkName2TargetIp.put(blkName, targetIP);
                        orderedReconWork.add(blkName2ReconWork.get(blkName));
                    }
                } catch (Exception e) {
                    LOG.info("Exception waiting for targets info");
                }

                // -- choose target for rw
                Set<DatanodeDescriptor> allDatanodes = getDatanodeManager().getDatanodes();
                Map<String, DatanodeDescriptor> dnIp2Descriptor = new HashMap<String, DatanodeDescriptor>();
                for(DatanodeDescriptor dn : allDatanodes) {
                    dnIp2Descriptor.put(dn.getIpAddr(), dn);
                }
                for (BlockReconstructionWork rw : reconWork) {
    
                    String blk = rw.getBlock().getBlockName();
        
                    DatanodeDescriptor dn = dnIp2Descriptor.get(blkName2TargetIp.get(blk));
                    // StorageType set as DISK
                    DatanodeStorageInfo linesDerterminTarget = dn.chooseStorage4Block(StorageType.DISK, rw.getBlockSize());

                    DatanodeStorageInfo[] linesTargets = {linesDerterminTarget};
                    rw.setTargets(linesTargets);
                }
                ...
                try {
                    // start time
                    long repairStartTime = Time.monotonicNow();
                    LOG.info("LinesYao repairStartTime: " + repairStartTime);
                    
                    for (BlockReconstructionWork rw : orderedReconWork) {
                        final DatanodeStorageInfo[] targets = rw.getTargets();
                        if (targets == null || targets.length == 0) {
                            blockLog.info("LinesYao need reset targets");
                            rw.resetTargets();
                            continue;
                        }
                        ...
                        synchronized (neededReconstruction) {
                            if (validateReconstructionWork(rw)) {
                                blockLog.info("LinesYao reconstructionwork for blk " + (new ExtendedBlock(getBlockPoolId(), (BlockInfoStriped) rw.getBlock())).getBlockName() + " scheduled");
                                scheduledWork++;
                            }
                        }
                    }
        ...
        ///
            ...
            // default step 2 & step 3 in hdfs

            // Step 2: choose target nodes for each reconstruction task
            final Set<Node> excludedNodes = new HashSet<>();
            for(BlockReconstructionWork rw : reconWork){ // Exclude all of the containing nodes from being targets.
                ...
                // choose replication targets: NOT HOLDING THE GLOBAL LOCK final
    ...
    BlockReconstructionWork scheduleReconstruction(BlockInfo block, int priority) {
        ...
        // get a source data-node //[linesyao]
        ...
        if (block.isStriped()) {
            ...
            return new ErasureCodingWork(getBlockPoolId(), block, bc, newSrcNodes, containingNodes, liveReplicaNodes,
                    additionalReplRequired, priority, newIndices);
        } else {
            return new ReplicationWork(block, bc, srcNodes, containingNodes, liveReplicaNodes, additionalReplRequired,
                    priority);
        }
    }

    /**
     * Adjust srcNodes and indices which are used to reconstruction block. We should
     * guarantee the indexes of first minRequiredSources nodes + are different.
     */
    ...
    private boolean validateReconstructionWork(BlockReconstructionWork rw) {
        ...
        if ((numReplicas.liveReplicas() >= requiredRedundancy) && (!placementStatus.isPlacementPolicySatisfied())) {
            ...
            if (!newPlacementStatus.isPlacementPolicySatisfied() && (newPlacementStatus
                    .getAdditionalReplicasRequired() >= placementStatus.getAdditionalReplicasRequired())) {
        ...
        // Add block to the datanode's task list [linesyao]!!!
     ...
     * @see BlockPlacementPolicy#chooseTarget(String, int, Node, Set, long, List,
     *      BlockStoragePolicy, EnumSet)
     */
    ...
    /**
     * Get the associated {@link DatanodeDescriptor} for the storage. If the storage
     * is of type PROVIDED, one of the nodes that reported PROVIDED storage are
     * returned. If not, this is equivalent to
    ...
    /**
     * Parse the data-nodes the block belongs to and choose a certain number from
     * them to be the recovery sources.
     *
     * We prefer nodes that are in DECOMMISSION_INPROGRESS state to other nodes
     * since the former do not have write traffic and hence are less busy. We do not
     * use already decommissioned nodes as a source, unless there is no other
     * choice. Otherwise we randomly choose nodes among those that did not reach
     * their replication limits. However, if the recovery work is of the highest
     ...
     * replication numbers.
     *
     * @param block                       Block for which a replication source is
     *                                    needed
     * @param containingNodes             List to be populated with nodes found to
     *                                    contain the given block
     * @param nodesContainingLiveReplicas List to be populated with nodes found to
     *                                    contain live replicas of the given block
     * @param numReplicas                 NumberReplicas instance to be initialized
     *                                    with the counts of live, corrupt, excess,
     *                                    and decommissioned replicas of the given
     *                                    block.
     * @param liveBlockIndices            List to be populated with indices of
     *                                    healthy blocks in a striped block group
     * @param priority                    integer representing replication priority
     *                                    of the given block
    ...
    DatanodeDescriptor[] chooseSourceDatanodes(BlockInfo block, List<DatanodeDescriptor> containingNodes,
        ...
        for (DatanodeStorageInfo storage : blocksMap.getStorages(block)) { // [linesyao] for all datanodestorageinfo
    ...
    /**
     * If there were any reconstruction requests that timed out, reap them and put
     * them back into the neededReconstruction queue
     */
        ...
        if (timedOutItems != null) {
            ...
            try {
                for (int i = 0; i < timedOutItems.length; i++) {
                    /*
                     * Use the blockinfo from the blocksmap to be certain we're working with the
                     * most up-to-date block information (e.g. genstamp).
                     */
            ...
            /*
             * If we know the target datanodes where the replication timedout, we could
             * invoke decBlocksScheduled() on it. Its ok for now.
             */
        }
    }
    ...
    /**
     * Set the total number of blocks in the system. If safe mode is not currently
     * on, this is a no-op.
     */
    ...
    void updateHeartbeat(DatanodeDescriptor node, StorageReport[] reports, long cacheCapacity, long cacheUsed,
     ...
     * updates to the information about under-construction blocks. Besides the block
     * in question, it provides the ReplicaState reported by the datanode in the
     * block report.
     */
    ...
    /**
     * The given storage is reporting all its blocks. Update the (storage-->block
     * list) and (block-->storage list) maps.
     *
    ...
    public boolean processReport(final DatanodeID nodeID, final DatanodeStorage storage,
            final BlockListAsLongs newReport, BlockReportContext context) throws IOException { // [linesyao] datanode processreport
        namesystem.writeLock();
        ...
        try {
            ...
            if (storageInfo.getBlockReportCount() == 0) {
                ...
                if (!StorageType.PROVIDED.equals(storageInfo.getStorageType())) {
                    invalidatedBlocks = processReport(storageInfo, newReport, context); // [linesyao]
                }
            }
    ...
    /**
     * Mark block replicas as corrupt except those on the storages in newStorages
     * list.
     */
    ...
    /**
     * Try to process any messages that were previously queued for the given block.
     * This is called from FSEditLogLoader whenever a block's state in the namespace
     * has changed or a new block has been created.
     */
    ...
    /**
     * Process any remaining queued datanode messages after entering active state.
     * At this point they will not be re-queued since we are the definitive master
     * node and thus should be up-to-date with the namespace information.
     */
    ...
    /**
     * The next two methods test the various cases under which we must conclude the
     * replica is corrupt, or under construction. These are laid out as switch
     * statements, on the theory that it is easier to understand the combinatorics
     * of reportedState and ucState that way. It should be at least as efficient as
     * boolean expressions.
     * 
                        ...
                        if (wrongSize) {
                            return new BlockToMarkCorrupt(new Block(reported), storedBlock,
                                    "block is " + ucState + " and reported length " + reported.getNumBytes()
                                            + " does not match " + "length in block map " + blockMapSize,
     ...
     * addStoredBlock(). Assumes this method is called "immediately" so there is no
     * need to refresh the storedBlock from blocksMap. Doesn't handle low
     * redundancy/extra redundancy, or worry about pendingReplications or
     * corruptReplicas, because it's in startup safe mode. Doesn't log every block,
     * because there are typically millions of them.
     * 
     * @throws IOException
     */
    ...
    /**
     * Modify (block-->datanode) map. Remove block from set of needed reconstruction
    ...
    /**
     * For each block in the name-node verify whether it belongs to any file, extra
     * or low redundancy. Place it into the respective queue.
     */
     ...
     * and supports further iteration after modification to list, there is a chance
     * of missing the newly added block while iterating. Since every addition to
     * blocksMap will check for mis-replication, missing mis-replication check for
     * new blocks will not be a problem.
     */
     ...
     * appropriate queues if necessary, and returns a result code indicating what
     * happened with it.
     */
     ...
     * extras, call chooseExcessRedundancies() to mark them in the
     * excessRedundancyMap.
     */
    ...
    /**
     * We want sufficient redundancy for the block, but we now have too many. In
     * this method, copy enough nodes from 'srcNodes' into 'dstNodes' such that:
     *
     * srcNodes.size() - dstNodes.size() == replication
     *
     * We pick node that make sure that replicas are spread across racks and also
     * try hard to pick one with least free space. The algorithm is first to pick a
     * node with least free space from nodes that are on a rack holding more than
     * one replicas of the block. So removing such a replica won't remove a rack. If
     * no such a node is available, then pick a node with least free space
     */
    ...
    /**
     * We want block group has every internal block, but we have redundant internal
     * blocks (which have the same index). In this method, we delete the redundant
     * internal blocks until only one left for each index.
     *
    ...
    public void removeStoredBlock(BlockInfo storedBlock, DatanodeDescriptor node) {
        ...
        assert (namesystem.hasWriteLock());
        {
            if (storedBlock == null || !blocksMap.removeNode(storedBlock, node)) {
                blockLog.debug("BLOCK* removeStoredBlock: {} has already been" + " removed from node {}", storedBlock,
                        node);
                return;
            }
            ...
            if (cblock != null) {
                ...
                if (removed) {
                    blockLog.debug("BLOCK* removeStoredBlock: {} removed from caching " + "related lists on node {}",
                            storedBlock, node);
                }
            }
     ...
     *         added block is a block group, return its approximate internal block
     *         size
     */
    ...
    /**
     * The given node is reporting incremental information about some blocks. This
     * includes blocks that are starting to be received, completed being received,
     * or deleted.
     * 
    ...
    private void processIncrementalBlockReport(final DatanodeDescriptor node, final StorageReceivedDeletedBlocks srdb)
            throws IOException {
     ...
     * those replicas. For a striped block, this includes nodes storing blocks
     * belonging to the striped block group. But note we exclude duplicated internal
     * block replicas for calculating {@link NumberReplicas#liveReplicas}. If the
     * replica on a decommissioning node is the same as the replica on a live node,
     * the internal block for this replica is live, not decommissioning.
     */
     ...
     * internal block. E.g., for the following list of internal blocks b0, b0, b1,
     * b2, b3, b4, b5, b6, b7 we have 9 internal blocks but we actually miss b8. We
     * should use this method to detect the above scenario and schedule necessary
     * reconstruction.
     */
    ...
    /**
     * Count distinct live and decommission internal blocks with blockIndex. If A
     * replica with INDEX is decommissioning, and B replica with INDEX is live, the
     * internal INDEX block is live.
     */
    private void countLiveAndDecommissioningReplicas(NumberReplicas counters, StoredReplicaState state,
            BitSet liveBitSet, BitSet decommissioningBitSet, byte blockIndex) {
        if (state == StoredReplicaState.LIVE) {
    ...
    /**
     * Simpler, faster form of {@link #countNodes} that only returns the number of
     * live nodes. If in startup safemode (or its 30-sec extension period), then it
     * gains speed by ignoring issues of excess replicas or nodes that are
    ...
    /**
     * On putting the node in service, check if the node has excess replicas. If
     * there are any excess replicas, call processExtraRedundancyBlock(). Process
     * extra redundancy blocks only when active NN is out of safe mode.
     */
    ...
    /**
     * Returns whether a node can be safely decommissioned or in maintenance based
     * on its liveness. Dead nodes cannot always be safely decommissioned or in
     * maintenance.
     */
    ...
    /**
     * Check sufficient redundancy of the blocks in the collection. If any block is
     * needed reconstruction, insert it into the reconstruction queue. Otherwise, if
     * the block is more than the expected replication factor, process it as an
     * extra redundancy block.
     */
    ...
    /**
     * Do some check when adding a block to blocksmap. For HDFS-7994 to check
     * whether then block is a NonEcBlockUsingStripedID.
     *
     */
    ...
    int computeDatanodeWork() {
        ...
        final int blocksToProcess = numlive * 1000;
        ///
    ...
    public static LocatedBlock newLocatedBlock(ExtendedBlock b, DatanodeStorageInfo[] storages, long startOffset,
        ...
        return new LocatedBlock(b, DatanodeStorageInfo.toDatanodeInfos(storages),
                DatanodeStorageInfo.toStorageIDs(storages), DatanodeStorageInfo.toStorageTypes(storages), startOffset,
                corrupt, null);
    }
    ...
    /**
     * Checks whether a recovery attempt has been made for the given block. If so,
     * checks whether that attempt has timed out.
     * 
     * @param b block for which recovery is being attempted
     * @return true if no recovery attempt has been made or the previous attempt
     *         timed out
     */
