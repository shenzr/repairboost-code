...
import org.apache.hadoop.hdfs.server.datanode.erasurecode.RepairBoostReconstructor;
///
...
public final class ErasureCodingWorker {
    ...
    //-[linesyao]
    private Boolean isRepairBoost;
    private Collection<BlockECReconstructionInfo> allEcTasks; // for full node
    private Map<String, RepairBoostReconstructor> lostFileName2EcTask;
    ...
    public ErasureCodingWorker(Configuration conf, DataNode datanode) {
        //-[linesyao]
        this.isRepairBoost = conf.getBoolean(
            DFSConfigKeys.DFS_DN_EC_REPAIRBOOST_KEY,
            DFSConfigKeys.DFS_DN_EC_REPAIRBOOST_DEFAULT
        );
        this.allEcTasks = new ArrayList<BlockECReconstructionInfo>();
        this.lostFileName2EcTask = new HashMap<String, RepairBoostReconstructor>();
        ...
        Preconditions.checkArgument(this.xmitWeight >= 0,
                "Invalid value configured for " + DFSConfigKeys.DFS_DN_EC_RECONSTRUCTION_XMITS_WEIGHT_KEY
    ...
    private void initializeStripedBlkReconstructionThreadPool(int numThreads) {
        ...
        stripedReconstructionPool = DFSUtilClient.getThreadPoolExecutor(2, numThreads, 60, new LinkedBlockingQueue<>(),
    ...
    public void processErasureCodingTasks(Collection<BlockECReconstructionInfo> ecTasks) {
        // [linesyao]
        // this.allEcTasks.addAll(ecTasks);
        LOG.info("LinesYao ecTasks.size = " + ecTasks.size());
        ///
        for (BlockECReconstructionInfo reconInfo : ecTasks) {
            ...
            try {
                StripedReconstructionInfo stripedReconInfo = new StripedReconstructionInfo(reconInfo.getExtendedBlock(),
                        reconInfo.getErasureCodingPolicy(), reconInfo.getLiveBlockIndices(),
                        reconInfo.getSourceDnInfos(), reconInfo.getTargetDnInfos(), reconInfo.getTargetStorageTypes(),
                        reconInfo.getTargetStorageIDs());
                // It may throw IllegalArgumentException from task#stripedReader
                // constructor.
                if(isRepairBoost) {
                    final RepairBoostReconstructor task = new RepairBoostReconstructor(this, stripedReconInfo);
                    if (task.hasValidTargets()) {
                        //-[linesyao]
                        LOG.info("LinesYao valid ecTask for blkName: " + reconInfo.getExtendedBlock().getBlockName());
                        xmitsSubmitted = Math.max((int) (task.getXmits() * xmitWeight), 1);
                        getDatanode().incrementXmitsInProcess(xmitsSubmitted);
                        stripedReconstructionPool.submit(task); // [linesyao]
                    } else {
