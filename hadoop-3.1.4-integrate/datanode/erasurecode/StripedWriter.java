...
class StripedWriter {
  ...
  //-[linesyao]
  private int lostFileNum = 0;
  private String[] lostFileNames = null;
  ///
  ...
  StripedWriter(StripedReconstructor reconstructor, DataNode datanode,
    ...
    //-[linesyao]
    this.lostFileNum = targetIndices.length;
    this.lostFileNames = new String[lostFileNum];
    LOG.info("LinesYao in StripedWriter: lostFileNum: " + this.lostFileNum);
    for(int i = 0; i < lostFileNum; i++){
      this.lostFileNames[i] = reconstructor.getBlock(targetIndices[i]).getBlockName();
      LOG.info("LinesYao in StripedWriter: lostFileName: " + this.lostFileNames[i]);
    }
    ///
  }
  //-[linesyao]
  String[] getLostFileName(){
    return this.lostFileNames;
  }
